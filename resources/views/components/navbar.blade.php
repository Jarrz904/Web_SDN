...
<nav x-data="{ mobileMenu: false, profilDropdown: false }" class="sticky top-0 z-50 bg-white shadow-sm py-4 px-6 md:px-20 flex justify-between items-center">
    ...
    <div class="hidden md:flex space-x-6 font-bold text-sm uppercase text-gray-600 items-center">
        <a href="{{ url('/') }}" class="{{ Request::is('/') ? 'text-green-500' : '' }} hover:text-green-500 transition">Home</a>

        <div class="relative group py-2">
            <button class="{{ Request::is('profil*', 'visi-misi*', 'guru*', 'struktur-organisasi*', 'data-siswa*') ? 'text-green-500' : 'text-slate-700' }} hover:text-green-500 flex items-center gap-1 transition font-bold">
                Profil <i class="fas fa-chevron-down text-[10px]"></i>
            </button>
            
            <div class="absolute hidden group-hover:block bg-white shadow-2xl p-4 w-60 rounded-xl top-full mt-0 border border-gray-100 z-[999]">
                <a href="{{ url('/visi-misi') }}" class="block py-2.5 px-4 rounded-lg {{ Request::is('visi-misi*') ? 'bg-green-50 text-green-500' : 'text-slate-600' }} hover:bg-green-50 hover:text-green-500 italic font-medium text-xs transition uppercase mb-1">
                    <i class="fas fa-bullseye mr-2"></i> Visi, Misi & Tujuan
                </a>
                <a href="{{ url('/profil') }}" class="block py-2.5 px-4 rounded-lg {{ Request::is('profil*') ? 'bg-green-50 text-green-500' : 'text-slate-600' }} hover:bg-green-50 hover:text-green-500 italic font-medium text-xs transition uppercase mb-1">
                    <i class="fas fa-school mr-2"></i> Profil Sekolah
                </a>
                <a href="{{ url('/guru') }}" class="block py-2.5 px-4 rounded-lg {{ Request::is('guru*') ? 'bg-green-50 text-green-500' : 'text-slate-600' }} hover:bg-green-50 hover:text-green-500 italic font-medium text-xs transition uppercase mb-1">
                    <i class="fas fa-user-tie mr-2"></i> Data Guru
                </a>
                <a href="{{ url('/struktur-organisasi') }}" class="block py-2.5 px-4 rounded-lg {{ Request::is('struktur-organisasi*') ? 'bg-green-50 text-green-500' : 'text-slate-600' }} hover:bg-green-50 hover:text-green-500 italic font-medium text-xs transition uppercase mb-1">
                    <i class="fas fa-sitemap mr-2"></i> Struktur Organisasi
                </a>
                <a href="{{ url('/data-siswa') }}" class="block py-2.5 px-4 rounded-lg {{ Request::is('data-siswa*') ? 'bg-green-50 text-green-500' : 'text-slate-600' }} hover:bg-green-50 hover:text-green-500 italic font-medium text-xs transition uppercase">
    ...
    <div x-show="mobileMenu" 
        ...
        <div>
            <button @click="profilDropdown = !profilDropdown" class="font-bold text-slate-700 flex justify-between w-full">
            ...
            <div x-show="profilDropdown" class="mt-2 pl-4 space-y-2 border-l-2 border-green-500">
                ...
                <a href="{{ url('/struktur-organisasi') }}" class="block text-xs uppercase text-slate-600">Struktur Organisasi</a>
